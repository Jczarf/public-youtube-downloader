...
RUN python -m pip install --no-cache-dir --only-binary=:all: "pip==26.2.1" \
    ...
    && python -c "import importlib.util; assert importlib.util.find_spec('pip') is None; assert importlib.util.find_spec('setuptools') is None; assert importlib.util.find_spec('msgpack') is None"
